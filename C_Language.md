...
中斷觸發：當中斷事件發生時，觸發中斷信號。這可以是硬體中斷（由硬體設備觸發）或軟體中斷（由指令或系統調用觸發）。

中斷請求（Interrupt Request, IRQ）：中斷觸發後，中斷控制器（例如在計算機系統中的硬體中斷控制器或在MCU中的中斷控制器）會發送中斷請求（IRQ）信號給處理器。

中斷服務例程（Interrupt Service Routine, ISR）註冊：處理器接收到中斷請求後，會在中斷向量表中查找相應的中斷服務例程（ISR）。中斷向量表是一個存儲著不同中斷號碼對應的ISR地址的資料結構。

中斷處理：處理器將執行中斷處理程序，即執行ISR。ISR是一個特定的程式段，負責處理中斷事件。在ISR中，可以進行各種操作，如保存上下文、處理中斷事件、更新狀態、執行相應的處理邏輯等。

中斷服務例程（ISR）結束：當ISR執行完成後，處理器返回到中斷點的下一條指令並恢復之前的執行狀態。這可能包括恢復被保存的上下文、恢復中斷許可狀態等。

繼續執行：處理器繼續執行被中斷打斷的原始程序，從中斷點下一條指令開始。
